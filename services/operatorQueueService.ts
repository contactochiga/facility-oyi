...
export async function loadOperatorQueue(role?: string | null): Promise<OperatorQueueItem[]> {
  const [maintenance, visitors, incidents, workflows, infrastructure, utilities, cameras] = await Promise.all([
  ...
  const items: OperatorQueueItem[] = [
    ...
    ...((utilities as any)?.items || []).filter((row: any) => /offline|failed|degraded|low|critical|warning/i.test(String(row.status || row.health || row.state || row.severity || ""))).map((row: any) => ({ id: `utility:${row.id || row.meter_id}`, title: row.name || row.utility_type || "Infrastructure service issue", module: "utilities" as const, priority: priority(`${row.status} ${row.health} ${row.severity}`), status: String(row.status || row.health || row.state || "attention"), owner: row.owner || "Unassigned", source: "Infrastructure services telemetry", created_at: row.observed_at || row.updated_at || row.created_at, route: "/services", next_action: "Review infrastructure service telemetry and provider status" })),
