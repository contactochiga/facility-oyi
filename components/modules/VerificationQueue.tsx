...
export default function VerificationQueue({ limit = 5, onSummary }: { limit?: number; onSummary?: (summary: VerificationSummaryValue) => void }) {
  ...
  return <OisCard as="section" className="h-full border-white/[0.06] bg-white/[0.024] p-3 sm:p-3.5"><div className="flex items-start justify-between gap-3"><div><h2 className="text-[15px] font-semibold tracking-[-0.02em] text-white">Verification Queue</h2><p className="mt-1 text-[11px] leading-4 text-zinc-500">Completed work awaiting authoritative confirmation.</p></div><Link href="/facility-intelligence?module=workflows" className="text-[11px] text-sky-200">Open queue</Link></div><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-zinc-500"><span><b className="mr-1 text-white">{summary.pending}</b>Pending</span><span><b className="mr-1 text-white">{summary.overdue}</b>Overdue</span><span><b className="mr-1 text-white">{summary.failed}</b>Failed</span><span><b className="mr-1 text-white">{summary.verifiedToday}</b>Today</span></div><div className="mt-2.5 space-y-1">{records.slice(0, limit).map(({ workflow, state, overdue }) => {
    return <button key={workflow.id || workflow.workflow_id} type="button" onClick={() => openWorkflowDrawer(String(workflow.id || workflow.workflow_id))} className="block w-full text-left">
      <OisListItem
        ...
        description={<span className="text-[11px] text-zinc-400">{String(state).replace(/_/g, " ")}</span>}
        meta={<span className="text-[10px] text-zinc-500">{formatDueState(workflow.workflow_due_at, overdue)}</span>}
