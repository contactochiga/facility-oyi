...
export default function VerificationQueue({ limit = 5, onSummary }: { limit?: number; onSummary?: (summary: VerificationSummaryValue) => void }) {
  ...
  return <OisCard as="section" className="border-white/[0.07] bg-white/[0.025] p-3.5 sm:p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold tracking-[-0.01em] text-white">Verification Queue</h2><p className="mt-1 text-[11px] text-zinc-600">Completed work awaiting authoritative confirmation.</p></div><Link href="/facility-intelligence?module=workflows" className="text-xs text-sky-200">Open queue</Link></div><div className="mt-3 space-y-1.5">{records.slice(0, limit).map(({ workflow, state, overdue }) => {
    return <button key={workflow.id || workflow.workflow_id} type="button" onClick={() => openWorkflowDrawer(String(workflow.id || workflow.workflow_id))} className="block w-full text-left">
      <OisListItem
        ...
        meta={<span className="text-[11px] text-zinc-600">{formatDueState(workflow.workflow_due_at, overdue)}</span>}
