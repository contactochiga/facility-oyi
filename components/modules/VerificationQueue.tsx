...
export default function VerificationQueue({ limit = 5, onSummary }: { limit?: number; onSummary?: (summary: VerificationSummaryValue) => void }) {
  ...
  return <OisCard as="section" className="p-3 sm:p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold text-white">Verification Queue</h2><p className="mt-1 text-xs text-zinc-500">Completed work awaiting authoritative confirmation.</p></div><Link href="/facility-intelligence?module=workflows" className="text-xs text-sky-200">Open queue</Link></div><div className="mt-3 space-y-1.5">{records.slice(0, limit).map(({ workflow, state, overdue }) => {
