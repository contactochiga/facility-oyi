...
export default function VerificationQueue({ limit = 5, onSummary }: { limit?: number; onSummary?: (summary: VerificationSummaryValue) => void }) {
  ...
  return <OisCard as="section" className="h-full border-white/[0.06] bg-white/[0.024] p-3 sm:p-3.5"><div className="flex items-start justify-between gap-3"><div><h2 className="text-[15px] font-semibold tracking-[-0.02em] text-white">Verification Queue</h2><p className="mt-1 text-[11px] leading-4 text-zinc-500">Completed work awaiting authoritative confirmation.</p></div><Link href="/facility-intelligence?module=workflows" className="text-[11px] text-sky-200">Open queue</Link></div><OisOperationalStrip className="mt-2.5" items={[{ label: "Pending", value: summary.pending, tone: "attention" }, { label: "Overdue", value: summary.overdue, tone: "warning" }, { label: "Failed", value: summary.failed, tone: "critical" }, { label: "Today", value: summary.verifiedToday, tone: "stable" }]} /><div className="mt-2.5 space-y-1">{records.slice(0, limit).map(({ workflow, state, overdue }) => {
