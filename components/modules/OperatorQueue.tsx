...
export default function OperatorQueue({ limit = 5 }: { limit?: number }) {
  ...
  return <OisCard as="section" className="h-full border-white/[0.06] bg-white/[0.024] p-3 sm:p-3.5"><div className="flex items-start justify-between gap-3"><div><h2 className="text-[15px] font-semibold tracking-[-0.02em] text-white">Operational Queue</h2><p className="mt-1 text-[11px] leading-4 text-zinc-500">Role-filtered work requiring ownership, verification, or the next action.</p></div><Link href="/facility-intelligence?module=workflows" className="text-[11px] text-sky-200">Open queue</Link></div><div className="mt-2.5 space-y-1">{items.slice(0, limit).map((item) => <Link key={item.id} href={item.route} onClick={(event) => { if (String(item.id).startsWith("workflow:")) { event.preventDefault(); openWorkflowDrawer(String(item.id).replace(/^workflow:/, "")); } }} className="block text-left">
    <OisListItem
      className="w-full"
      title={<span className="block truncate text-sm font-medium text-white">{item.title}</span>}
      description={<span className="text-[11px] text-zinc-400">{item.status}</span>}
      meta={<span className="text-[10px] text-zinc-500">{item.route.includes("workflow") ? "Review ownership" : "Inspect device health"}</span>}
