...
export default function OperatorQueue() {
  ...
  return <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold text-white">Operator Queue</h2><p className="mt-1 text-xs text-zinc-500">Role-filtered work needing attention, ownership, or verification.</p></div><Link href="/facility-intelligence?module=workflows" className="text-xs text-sky-200">Ask Oyi</Link></div><div className="mt-4 grid grid-cols-4 gap-2 text-center text-xs"><div className="rounded-xl bg-black/20 p-2"><b className="block text-white">{summary.open}</b>Open</div><div className="rounded-xl bg-black/20 p-2"><b className="block text-amber-100">{summary.overdue}</b>Overdue</div><div className="rounded-xl bg-black/20 p-2"><b className="block text-orange-100">{summary.unassigned}</b>Unassigned</div><div className="rounded-xl bg-black/20 p-2"><b className="block text-sky-100">{summary.verification}</b>Verify</div></div><div className="mt-4 space-y-2">{items.slice(0, 8).map((item) => <Link key={item.id} href={item.route} className="block rounded-xl border border-white/10 bg-black/20 p-3 hover:bg-white/[0.05]"><div className="flex justify-between gap-2"><span className="truncate text-sm font-medium text-white">{item.title}</span><span className={`shrink-0 rounded-full border px-2 py-1 text-[10px] ${tone[item.priority]}`}>{item.priority}</span></div><p className="mt-1 text-xs text-zinc-500">{item.module} · {item.status} · {item.owner} · {age(item.created_at)}</p>{item.blocking_reason ? <p className="mt-1 text-xs text-amber-100">Blocked: {item.blocking_reason}</p> : null}{item.verification === "pending" ? <p className="mt-1 text-xs text-sky-100">Verification required</p> : null}<p className="mt-2 text-xs text-sky-100">Next: {item.next_action}</p></Link>)}{!items.length && !loading ? <p className="rounded-xl border border-dashed border-white/10 p-4 text-sm text-zinc-500">No unresolved operator items are visible for this role.</p> : null}</div></section>;
}
