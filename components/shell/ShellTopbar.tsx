...
export default function ShellTopbar() {
  ...
  const estateLabel = useMemo(
    () => String(context?.estate?.name || "Estate Context"),
    [context?.estate?.name]
  );
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-[var(--ois-border-subtle)] bg-[rgba(7,10,15,0.92)] backdrop-blur-xl">
        <div className="mx-auto flex h-[68px] max-w-[1680px] items-center gap-3 px-3 sm:px-5 xl:px-7">
          <button
            type="button"
          ...
          <button
            ...
            className={cn(
              "flex h-11 flex-1 items-center justify-between rounded-[14px] border border-[var(--ois-border-default)] bg-[var(--ois-surface)] px-3 text-left text-sm text-[var(--ois-text-muted)]",
