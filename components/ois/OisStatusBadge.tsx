...
export default function OisStatusBadge({ status, label, size = "sm", className = "" }: { status: OisStatus | string; label?: string; size?: "sm" | "md"; className?: string }) {
  ...
  return <span className={`inline-flex items-center rounded-[var(--ois-radius-pill)] border font-medium tracking-[0.02em] ${size === "md" ? "px-2 py-0.5 text-[10px] leading-4" : "px-1.5 py-px text-[9px] leading-4"} ${className}`} style={{ color: `var(--ois-status-${token})`, backgroundColor: `var(--ois-status-${token}-surface)`, borderColor: `var(--ois-status-${token}-border)` }}>{label || labels[normalized]}</span>;
}
