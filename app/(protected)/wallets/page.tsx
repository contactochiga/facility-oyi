...
export default function WalletsPage() {
  ...
  if (!canRead) {
    return <div className="space-y-6"><Topbar title="Finance" subtitle="Finance access required" /><div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 p-5 text-sm text-amber-100">Permission required: wallets.read.</div></div>;
  }

  return (
    <div className="space-y-6">
      <Topbar title="Finance" subtitle="Transactions and payment review" strip={[{ label: "Healthy", value: pending.length || failed.length ? "Review" : "Stable", detail: "Financial posture", tone: pending.length || failed.length ? "warning" : "stable" }, { label: "Payments", value: rows.length, detail: "Visible transactions", tone: "attention" }, { label: "Attention", value: pending.length + failed.length, detail: "Pending or failed", tone: "warning" }, { label: "Updated", value: loading ? "Refreshing" : "Now", detail: "Runtime sync", tone: "info" }]} />
