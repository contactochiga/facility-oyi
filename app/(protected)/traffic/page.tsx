...
import { Activity, AlertTriangle, CheckCircle2, Clock3, DoorOpen, KeyRound, ListChecks, RefreshCw, ShieldCheck, Users } from "lucide-react";
...
type Tab = "dashboard" | "access-points" | "gate-control" | "access-logs";
const tabs: Array<{ key: Tab | "visitors"; label: string; href?: string }> = [
  { key: "dashboard", label: "Dashboard" }, { key: "visitors", label: "Visitors", href: "/visitors" },
...
export default function TrafficPage() {
  ...
  return <div className="space-y-3.5 pb-6">
    ...
    <div className="flex justify-end"><Button variant="ghost" className="h-8 gap-2 rounded-md px-3 text-[10px]" onClick={() => void load()}><RefreshCw className={`h-3.5 w-3.5 ${loading ? "animate-spin" : ""}`}/>Refresh</Button></div>
    ...
    <nav className="flex gap-5 overflow-x-auto border-b border-[var(--ois-border-subtle)] px-1">{tabs.map(item => item.href ? <Link key={item.key} href={item.href} className="shrink-0 pb-2.5 text-[10px] text-zinc-500 hover:text-white">{item.label}</Link> : <button key={item.key} type="button" onClick={() => setTab(item.key as Tab)} className={`relative shrink-0 pb-2.5 text-[10px] ${tab === item.key ? "text-sky-200 after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-sky-400" : "text-zinc-500 hover:text-white"}`}>{item.label}</button>)}</nav>

    {tab === "dashboard" ? <section className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_300px]"><main className="grid gap-4"><Panel title="Live Access Overview" caption="Real-time access activity across the facility"><VisitorRows rows={recent} homeLabels={homeLabels}/></Panel><Panel title="Upcoming Visits" caption="Scheduled and pre-approved visits"><div className="mt-3 hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] text-left"><thead className="text-[8.5px] uppercase tracking-[.09em] text-zinc-600"><tr><th className="pb-2.5">Visitor</th><th>Purpose</th><th>Host / unit</th><th>Schedule</th><th>Access point</th><th>Status</th><th>Action</th></tr></thead><tbody className="divide-y divide-[var(--ois-border-subtle)]">{upcoming.map(visitor => <tr key={visitor.id} className="text-[10px] text-zinc-400"><td className="py-2.5 font-medium text-zinc-100">{visitor.visitor_name}</td><td>{visitor.purpose || "Not recorded"}</td><td>{visitor.home_id ? homeLabels.get(visitor.home_id) || "Home assigned" : "Not assigned"}</td><td>{when(visitor.expires_at)}</td><td>Not assigned</td><td><OisStatusBadge status={tone(status(visitor)) as any} label={status(visitor)}/></td><td><Link href="/visitors" className="text-sky-300">Review</Link></td></tr>)}</tbody></table></div><div className="mt-3 divide-y divide-[var(--ois-border-subtle)] md:hidden">{upcoming.map(visitor => <Link href="/visitors" key={visitor.id} className="block py-3"><b className="text-[10.5px] text-zinc-100">{visitor.visitor_name}</b><small className="block text-[8.5px] text-zinc-600">{visitor.purpose || "Visit"} · {when(visitor.expires_at)}</small></Link>)}</div>{!upcoming.length ? <p className="py-10 text-center text-[10.5px] text-zinc-600">No upcoming visits.</p> : null}</Panel></main><aside className="grid gap-4"><Panel title="Gate Status" caption="All access points and their current status"><div className="mt-3 space-y-1.5">{accessPoints.slice(0, 8).map(device => <div key={device.id} className="flex items-center gap-2 rounded-lg border border-[var(--ois-border-subtle)] px-3 py-2.5"><DoorOpen className="h-3.5 w-3.5 text-sky-300"/><span className="min-w-0 flex-1"><b className="block truncate text-[10.5px] font-medium text-zinc-200">{device.name}</b><small className="text-[8.5px] text-zinc-600">{device.device_type || device.category || "Access point"}</small></span><OisStatusBadge status={tone(deviceState(device)) as any} label={deviceState(device)}/></div>)}{!accessPoints.length ? <p className="py-8 text-center text-[10.5px] text-zinc-600">No access points configured.</p> : null}</div></Panel><Panel title="Access Methods" caption="Credential usage from canonical access records"><div className="py-8 text-center"><KeyRound className="mx-auto h-5 w-5 text-zinc-700"/><p className="mt-2 text-[10.5px] text-zinc-500">Access-method aggregation unavailable</p><small className="text-[8.5px] text-zinc-600">Visitor records do not expose a method field.</small></div></Panel><Panel title="Quick Actions" caption="Common access operations"><div className="mt-3 grid grid-cols-2 gap-1.5"><Link href="/visitors" className="flex h-10 items-center gap-2 rounded-lg border border-[var(--ois-border-subtle)] px-2.5 text-[9.5px] text-zinc-300"><KeyRound className="h-3.5 w-3.5 text-sky-300"/>Verify visitor</Link><Link href="/visitors" className="flex h-10 items-center gap-2 rounded-lg border border-[var(--ois-border-subtle)] px-2.5 text-[9.5px] text-zinc-300"><Users className="h-3.5 w-3.5 text-sky-300"/>All visitors</Link><button type="button" onClick={() => setTab("access-logs")} className="flex h-10 items-center gap-2 rounded-lg border border-[var(--ois-border-subtle)] px-2.5 text-[9.5px] text-zinc-300"><ListChecks className="h-3.5 w-3.5 text-sky-300"/>Access logs</button><button type="button" onClick={() => setTab("access-points")} className="flex h-10 items-center gap-2 rounded-lg border border-[var(--ois-border-subtle)] px-2.5 text-[9.5px] text-zinc-300"><DoorOpen className="h-3.5 w-3.5 text-sky-300"/>Access points</button></div></Panel></aside></section> : null}
    ...
    {tab === "access-logs" ? <Panel title="Access Logs" caption="Bounded canonical visitor lifecycle records"><VisitorRows rows={allVisitors.slice().sort((a,b)=>dateValue(b.created_at)-dateValue(a.created_at)).slice(0,40)} homeLabels={homeLabels}/></Panel> : null}
  </div>;
}
