...
const TABS: Array<{ key: Tab; label: string; icon: typeof Users }> = [
  ...
  { key: "settings", label: "Estate Settings", icon: Settings },
];

const ROLE_DEFINITIONS = [
  { key: "estate_admin", label: "Estate Admin", description: "Owns estate-level administration, homes, staff, residents and operational governance.", inheritance: "Top estate role", scope: "Estate-wide" },
...
function statusTone(value?: string | null) {
  ...
  if (/active|connected|ready|configured|enabled|sent|live/.test(status)) return "border-emerald-500/20 bg-emerald-500/10 text-emerald-200";
  if (/suspended|removed|revoked|failed|error|disconnected|expired/.test(status)) return "border-rose-500/20 bg-rose-500/10 text-rose-200";
  if (/pending|awaiting|unknown|invited|no configuration|not configured/.test(status)) return "border-amber-500/20 bg-amber-500/10 text-amber-100";
  return "border-white/10 bg-white/5 text-zinc-300";
}

function Status({ value }: { value: string }) {
  return <span className={`rounded-full border px-2 py-1 text-[10px] uppercase tracking-[0.12em] ${statusTone(value)}`}>{value}</span>;
}

function Metric({ label, value, hint }: { label: string; value: string | number; hint: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><p className="text-[10px] uppercase tracking-[0.17em] text-zinc-500">{label}</p><p className="mt-3 text-2xl font-semibold tracking-tight text-white">{value}</p><p className="mt-2 text-xs leading-5 text-zinc-500">{hint}</p></div>;
}

function Panel({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 sm:p-5"><h2 className="text-sm font-semibold text-white">{title}</h2>{subtitle ? <p className="mt-1 text-xs leading-5 text-zinc-500">{subtitle}</p> : null}<div className="mt-4">{children}</div></section>;
}

function Field({ label, value }: { label: string; value: React.ReactNode }) {
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-[10px] uppercase tracking-[0.14em] text-zinc-500">{label}</p><div className="mt-1 text-sm text-zinc-200">{value}</div></div>;
}
...
export default function FacilityAdministrationModule() {
  ...
  const integrations = useMemo(() => {
    ...
    return [
      { name: "Tuya / Smart Life", status: tuya ? text(tuya.status || tuya.connection_status, "Connected") : "Pending configuration", detail: tuya ? text(tuya.last_sync_at || tuya.updated_at, "Last sync unavailable") : "Provider configured through device provider sync." },
  ...
  return (
    <div className="space-y-6">
      <Topbar title="Facility Administration" subtitle="Operational governance center for operators, roles, permissions, audit, integrations, notifications, security and estate settings." rightSlot={<Button variant="ghost" onClick={() => void load()} disabled={loading} className="gap-2"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />Refresh</Button>} />
      ...
      <section className="rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,0.12),transparent_34%),linear-gradient(145deg,rgba(255,255,255,0.055),rgba(255,255,255,0.018))] p-5">
        <p className="text-[10px] uppercase tracking-[0.18em] text-sky-200/80">Administration governance</p>
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-white">{estate?.name || "Estate administration"}</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-400">Manage Facility operators and inspect governance state using live contracts. Unsupported custom-role, delivery-metric, provider-config and branding edits remain read-only pending backend support.</p>
      </section>
...
function OperatorsSection({ operators, source, canManage, pendingOperators, onOpen }: { operators: EstateMembershipRow[]; source: Source<EstateMembershipRow[]>; canManage: boolean; pendingOperators: number; onOpen: (detail: Detail) => void }) {
  if (source.status !== "ready") return <Panel title="Operator Management" subtitle="Operator source state"><p className="rounded-xl border border-dashed border-white/10 p-5 text-sm text-zinc-500">{sourceLabel(source, "Awaiting activity source")}</p></Panel>;
  return <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]"><Panel title="Operator Management" subtitle="Estate operators, roles, status, assigned scope and honest activity state."><div className="space-y-2">{operators.map((member) => <article key={member.id} className="rounded-2xl border border-white/10 bg-black/15 p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><h3 className="text-sm font-semibold text-white">{operatorName(member)}</h3><p className="mt-1 text-xs text-zinc-500">{operatorEmail(member)} · {member.role || "operator"}</p></div><div className="flex flex-wrap gap-2"><Status value={member.status || "unknown"} /><Button variant="ghost" onClick={() => onOpen({ title: operatorName(member), subtitle: operatorEmail(member), rows: [["Role", member.role || "operator"], ["Status", member.status || "unknown"], ["Assigned scope", "Estate scope"], ["Last login", "Awaiting activity source"], ["Activity", "Awaiting activity source"], ["Suspension state", /suspended/.test(lower(member.status)) ? "Suspended" : "Not suspended"]], href: "/homes" })} className="gap-2"><Eye className="h-4 w-4" />Inspect</Button></div></div></article>)}{!operators.length ? <p className="rounded-xl border border-dashed border-white/10 p-5 text-sm text-zinc-500">No operators returned by the estate user source.</p> : null}</div></Panel><Panel title="Lifecycle Controls" subtitle="Mutation support is limited to existing backend contracts."><div className="space-y-3 text-sm text-zinc-400"><Field label="Pending operators" value={String(pendingOperators)} /><Field label="Role assignment" value={canManage ? "Supported through estate/home membership update workflows" : "Permission required: staff.manage"} /><Field label="Suspend / remove" value={canManage ? "Supported where backend membership routes permit it" : "Permission required: staff.manage"} /><Field label="Last login" value="Awaiting activity source" /></div></Panel></section>;
}
...
function AuditSection({ source, rows }: { source: Source<any[]>; rows: any[] }) {
  ...
  return <Panel title="Audit Center" subtitle="Authentication, resident lifecycle, invitations, devices, visitors, moderation and administrative actions from backend audit logs."><div className="space-y-2">{rows.map((item) => <article key={item.id || `${item.action}-${item.created_at}`} className="rounded-xl border border-white/10 bg-black/15 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-white">{item.action || "Audit event"}</h3><p className="mt-1 text-xs text-zinc-500">{item.target_type || "target"}:{item.target_id || "n/a"} · actor:{item.actor_role || "n/a"}</p></div><Status value={item.status || "recorded"} /></div><p className="mt-2 text-xs text-zinc-500">{dateLabel(item.created_at)}</p></article>)}{!rows.length ? <p className="rounded-xl border border-dashed border-white/10 p-5 text-sm text-zinc-500">No audit entries match this filter.</p> : null}</div></Panel>;
}
...
function IntegrationsSection({ rows, infra }: { rows: Array<{ name: string; status: string; detail: string }>; infra: Source<InfrastructureOperations | null> }) {
  return <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]"><Panel title="Integrations Management" subtitle="Provider readiness without exposing secrets or unsupported connection states."><div className="grid gap-3 lg:grid-cols-2">{rows.map((item) => <article key={item.name} className="rounded-2xl border border-white/10 bg-black/15 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-white">{item.name}</h3><p className="mt-2 text-xs leading-5 text-zinc-500">{item.detail}</p></div><Status value={item.status} /></div></article>)}</div></Panel><Panel title="Provider Source" subtitle="Backend provider and Edge source availability."><div className="space-y-2 text-sm text-zinc-400"><Field label="Infrastructure source" value={infra.status === "ready" ? "Available" : sourceLabel(infra)} /><Field label="Provider sync" value={infra.data?.sources?.providers?.available ? "Connected" : "Pending configuration"} /><Field label="Oyi Edge" value={infra.data?.edge_nodes?.length ? `${infra.data.edge_nodes.length} node(s)` : "Pending configuration"} /></div></Panel></section>;
}
...
function SecuritySection({ userRole, canSettings, canAudit, audit, operators }: { userRole: string; canSettings: boolean; canAudit: boolean; audit: Source<any[]>; operators: Source<EstateMembershipRow[]> }) {
  return <section className="grid gap-5 xl:grid-cols-2"><Panel title="Security Policies" subtitle="Authentication, session, role and operator posture."><div className="grid gap-3 sm:grid-cols-2"><Field label="Authentication posture" value="JWT-protected Facility routes" /><Field label="Session posture" value="Cookie/local token cleared on logout by protected shell" /><Field label="Operator access" value={operators.status === "ready" ? `${operators.data.length} estate memberships` : sourceLabel(operators)} /><Field label="Current operator role" value={userRole.replace(/_/g, " ")} /><Field label="Settings permission" value={canSettings ? "settings.manage available" : "Permission required"} /><Field label="Audit permission" value={canAudit ? "audit.read available" : "Permission required"} /></div></Panel><Panel title="Security Events" subtitle="Audit-backed security visibility."><p className="rounded-xl border border-dashed border-white/10 p-5 text-sm text-zinc-500">{audit.status === "ready" ? `${audit.data.length} audit entries loaded. Use Audit tab for filtering.` : sourceLabel(audit, "Awaiting security event source")}</p></Panel></section>;
}

function EstateSettingsSection({ estate, source, canSettings }: { estate: any; source: Source<any[]>; canSettings: boolean }) {
  if (source.status !== "ready") return <Panel title="Estate Settings" subtitle="Estate settings source"><p className="rounded-xl border border-dashed border-white/10 p-5 text-sm text-zinc-500">{sourceLabel(source)}</p></Panel>;
  return <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]"><Panel title="Estate Settings" subtitle="Operational settings center. Editing is only enabled where backend persistence exists."><div className="grid gap-3 sm:grid-cols-2"><Field label="Estate name" value={estate?.name || "Pending source"} /><Field label="Address" value={estate?.address || "Pending source"} /><Field label="Type" value={estate?.type || "Pending source"} /><Field label="Timezone" value="Pending backend support" /><Field label="Branding" value="Pending backend support" /><Field label="Communication settings" value="Pending backend support" /><Field label="Access settings" value="Managed through Homes/Members access workflows" /><Field label="Operational settings" value="Pending backend support" /></div></Panel><Panel title="Settings Controls" subtitle="Fail-closed editing posture."><div className="space-y-3 text-sm text-zinc-400"><Field label="Edit permission" value={canSettings ? "settings.manage available" : "Permission required: settings.manage"} /><Field label="Persistence" value="Estate create/update fields are available in Homes/Estate workflows. Branding/timezone/communication settings need backend contracts." /><Button variant="ghost" disabled={!canSettings}>Configuration editor pending backend support</Button></div></Panel></section>;
}
