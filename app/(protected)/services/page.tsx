...
function UtilitiesRegistryWorkspace({ accounts, configs, transactions, providers, filtered, activeDomain, setActiveDomain, search, setSearch, loading, attentionCount, onRefresh, onSelectAccount, onSelectPolicy }: {
  ...
  const homes = useMemo(() => {
    const groups = new Map<string, InfrastructureServiceAccountRow[]>();
    filtered.forEach((account) => {
      const key = account.home_id ? `home:${account.home_id}` : `account:${account.id}`;
      groups.set(key, [...(groups.get(key) || []), account]);
    });
    return [...groups.entries()].map(([key, services]) => {
      const activeServices = services.filter((item) => /ready|active|stable/.test(`${item.status} ${item.vending_readiness}`.toLowerCase())).length;
      const attentionServices = services.filter((item) => /attention|manual_review|failed|error|offline|unsupported/.test(`${item.status} ${item.vending_readiness} ${item.provider_health}`.toLowerCase())).length;
      const lastActivity = services.map((item) => item.last_activity_at).filter(Boolean).sort().at(-1) || null;
      return { key, label: textFor(services[0]?.home_label, services[0]?.home_id ? "Home assigned" : "Unassigned service"), services, active: activeServices, pending: Math.max(0, services.length - activeServices - attentionServices), attention: attentionServices, lastActivity };
    }).sort((a, b) => a.label.localeCompare(b.label));
  }, [filtered]);
  function toggleHome(key: string) { setExpandedHomes(current => { const next = new Set(current); if (next.has(key)) next.delete(key); else next.add(key); return next; }); }
  return (
    <section className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
      <div className="rounded-xl border border-white/[0.075] bg-white/[0.025] p-4">
        ...
        <div className="mt-4 hidden grid-cols-[minmax(180px,1.4fr)_70px_62px_62px_70px_105px_24px] gap-3 border-b border-white/[0.06] px-2 pb-2 text-[9px] uppercase tracking-[0.12em] text-zinc-600 lg:grid"><span>Home / location</span><span>Total</span><span>Active</span><span>Pending</span><span>Attention</span><span>Last activity</span><span /></div>
        <div className="divide-y divide-white/[0.055]">{homes.map(home => { const expanded = expandedHomes.has(home.key); return <section key={home.key} data-utility-home={home.key} className="py-0.5"><button type="button" aria-expanded={expanded} onClick={() => toggleHome(home.key)} className="grid w-full gap-2 px-2 py-3 text-left hover:bg-white/[0.02] lg:grid-cols-[minmax(180px,1.4fr)_70px_62px_62px_70px_105px_24px] lg:items-center lg:gap-3"><span className="flex min-w-0 items-center gap-2.5"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-white/[0.07] bg-white/[0.025] text-sky-300"><Home className="h-4 w-4" /></span><span className="min-w-0"><b className="block truncate text-[11.5px] font-medium text-white">{home.label}</b><small className="block truncate text-[9.5px] text-zinc-600">{home.services.length} service{home.services.length === 1 ? "" : "s"}</small></span></span><span className="text-[10.5px] text-zinc-300"><small className="mr-1 text-zinc-600 lg:hidden">Total</small>{home.services.length}</span><span className="text-[10.5px] text-emerald-300"><small className="mr-1 text-zinc-600 lg:hidden">Active</small>{home.active}</span><span className="text-[10.5px] text-amber-300"><small className="mr-1 text-zinc-600 lg:hidden">Pending</small>{home.pending}</span><span className={home.attention ? "text-[10.5px] text-rose-300" : "text-[10.5px] text-zinc-500"}><small className="mr-1 text-zinc-600 lg:hidden">Attention</small>{home.attention}</span><span className="text-[10px] text-zinc-500">{when(home.lastActivity)}</span>{expanded ? <ChevronDown className="h-3.5 w-3.5 text-zinc-500" /> : <ChevronRight className="h-3.5 w-3.5 text-zinc-500" />}</button>{expanded ? <div className="ml-4 border-l border-sky-400/15 pl-3 sm:ml-6"><div className="hidden grid-cols-[minmax(180px,1.3fr)_1fr_.9fr_.75fr_.8fr_52px] gap-3 border-y border-white/[0.045] px-2 py-1.5 text-[8px] uppercase tracking-[.09em] text-zinc-600 lg:grid"><span>Service / account</span><span>Provider</span><span>Billing</span><span>Status</span><span>Last activity</span><span>Action</span></div>{home.services.map(account => <button key={account.id} type="button" onClick={() => onSelectAccount(account)} className="grid w-full gap-2 border-t border-white/[0.045] px-2 py-2.5 text-left first:border-t-0 hover:bg-white/[0.02] lg:grid-cols-[minmax(180px,1.3fr)_1fr_.9fr_.75fr_.8fr_52px] lg:items-center lg:gap-3"><span className="flex min-w-0 items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-md bg-white/[0.025] text-sky-300"><UtilityIcon serviceKey={account.service_key} /></span><span className="min-w-0"><b className="block truncate text-[10.5px] font-medium text-zinc-200">{SERVICE_LABELS[account.service_key] || account.service_title}</b><small className="block truncate text-[8.5px] text-zinc-600">{textFor(account.identifier || account.meter_number || account.account_number, "Account not configured")}</small></span></span><span className="truncate text-[10px] text-zinc-400">{textFor(account.provider)}</span><span className="text-[10px] text-zinc-400">{textFor(account.billing_profile)}</span><span><OisStatusBadge status={toneFor(account.status || account.vending_readiness)} label={textFor(account.status || account.vending_readiness, "Pending")} /></span><span className="text-[9.5px] text-zinc-500">{when(account.last_activity_at)}</span><span className="text-[10px] text-sky-300">View</span></button>)}</div> : null}</section>; })}{!homes.length && !loading ? <div className="py-16 text-center"><Gauge className="mx-auto h-6 w-6 text-zinc-700" /><p className="mt-3 text-xs text-zinc-400">No utility services match this view</p><p className="mt-1 text-[10px] text-zinc-600">Configured service accounts will appear here.</p></div> : null}</div>
      </div>
