...
export default function CamerasPage() {
  ...
  return (
    <div className="space-y-6">
      <Topbar title="Camera Operations" subtitle="Cameras and stream health" strip={[{ label: "DVRs", value: loading ? "Loading" : summary.dvrs, detail: "Registered sources", tone: "attention" }, { label: "Cameras", value: summary.cameras, detail: "Imported channels", tone: "attention" }, { label: "Attention", value: summary.offline_streams, detail: "Offline streams", tone: summary.offline_streams ? "warning" : "stable" }, { label: "AI", value: summary.ai_enabled_cameras, detail: "Profiles enabled", tone: "info" }]} rightSlot={<div className="flex gap-2"><Button variant="ghost" onClick={() => void load()} disabled={loading} className="gap-2"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Refresh</Button><Button onClick={() => { resetImport(); setImportOpen(true); }} className="gap-2"><Plus className="h-4 w-4" /> Import DVR/NVR</Button></div>} />
