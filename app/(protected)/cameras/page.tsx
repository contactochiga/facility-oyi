...
export default function CamerasPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((camera) => {
      ...
      const scope = camera.scope as CameraPrivacyScope;
      ...
      const haystack = [camera.name, camera.ip, camera.location, camera.nvrId, camera.channel, camera.edgeNodeId].map((x) => String(x || "").toLowerCase()).join(" ");
  ...
  async function scan() {
    ...
    setError(null);
    try {
      if (!estateId || !edgeNodeId) throw new Error("Select an estate and enter its Oyi Edge node ID.");
      const result = await cameraService.scan({ estateId, edgeNodeId, cidr, credentialRef: credentialRef || undefined });
      setScanResults(result.items || []);
      if (!result.items?.length) setNotice("No cameras were returned by the selected Oyi Edge node.");
    } catch (requestError: any) {
  ...
  async function bind() {
    if (!binding) return;
    try {
      await cameraService.provision(binding.discoveryId, { name: bindName || binding.name || "Camera", location: location || undefined, scope: "facility", credentialRef: credentialRef || undefined });
      setBinding(null);
      setScanOpen(false);
      setPassword("");
      setUsername("");
      setNotice("Camera provisioned into the canonical estate registry.");
      await load();
    } catch (requestError: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      {scanOpen ? <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-4 backdrop-blur-sm"><section className="w-full max-w-3xl rounded-2xl border border-white/10 bg-zinc-950 p-5"><header className="flex justify-between gap-3"><div><h2 className="text-lg font-semibold text-white">Discover standalone camera</h2><p className="mt-1 text-sm text-zinc-500">An authorized Oyi Edge node performs private-network discovery.</p></div><button type="button" onClick={() => setScanOpen(false)}><X className="h-4 w-4 text-zinc-400" /></button></header><div className="mt-5 grid gap-3 sm:grid-cols-3"><input value={cidr} onChange={(e) => setCidr(e.target.value)} placeholder="Private subnet" className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white" /><input value={edgeNodeId} onChange={(e) => setEdgeNodeId(e.target.value)} placeholder="Oyi Edge node ID" className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white" /><input value={credentialRef} onChange={(e) => setCredentialRef(e.target.value)} placeholder="Edge credential reference" className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white" /></div><Button onClick={() => void scan()} disabled={scanning || !estateId || !edgeNodeId} className="mt-4 gap-2"><Search className="h-4 w-4" /> {scanning ? "Waiting for Edge…" : "Run Edge discovery"}</Button><div className="mt-4 grid gap-2">{scanResults.map((camera) => <button key={camera.discoveryId} type="button" onClick={() => { setBinding(camera); setBindName(camera.name || ""); }} className="rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-left hover:border-sky-400/30"><p className="text-sm text-white">{camera.name}</p><p className="text-xs text-zinc-500">{camera.provider} · {camera.onvifAvailable ? "ONVIF available" : "ONVIF unavailable"} · {camera.rtspAvailable ? "Stream available at Edge" : "Stream unavailable"}</p></button>)}</div></section></div> : null}
      {binding ? <div className="fixed inset-0 z-[60] grid place-items-center bg-black/75 p-4 backdrop-blur-sm"><section className="w-full max-w-xl rounded-2xl border border-white/10 bg-zinc-950 p-5"><h2 className="text-lg font-semibold text-white">Provision camera</h2><div className="mt-5 grid gap-3 sm:grid-cols-2"><input value={bindName} onChange={(e) => setBindName(e.target.value)} placeholder="Camera name" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" /><input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Location e.g. Main Gate" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" /><input value={credentialRef} onChange={(e) => setCredentialRef(e.target.value)} placeholder="Edge credential reference" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none sm:col-span-2" /></div><p className="mt-3 text-xs text-zinc-500">{binding.provider} · Private connectivity and credentials remain on Oyi Edge.</p><footer className="mt-5 flex justify-end gap-2"><Button variant="ghost" onClick={() => setBinding(null)}>Cancel</Button><Button onClick={() => void bind()} disabled={binding.requiresAuthentication && !credentialRef}><Link2 className="mr-2 h-4 w-4" /> Provision Camera</Button></footer></section></div> : null}
      <OisDrawer open={Boolean(playing)} onClose={() => setPlaying(null)} title={playing?.name || playing?.ip || "Camera playback"} subtitle={playing ? `${text(playing.location || playing.ip, "Camera source")} · ${privacyLabel(playing.scope)}` : undefined} width="lg">
