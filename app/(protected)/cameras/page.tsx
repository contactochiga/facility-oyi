  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border border-white/10"><div className="grid grid-cols-7 gap-3 border-b border-white/10 bg-white/[0.03] px-4 py-3 text-[10px] uppercase tracking-[0.16em] text-zinc-500"><span>DVR</span><span>Channel</span><span>Location</span><span>Status</span><span>Privacy</span><span>Edge</span><span>Last seen</span></div>{filtered.map((camera) => { const s = state(camera); return <article key={camera.id} className="grid grid-cols-7 gap-3 border-b border-white/5 px-4 py-3 text-sm last:border-b-0"><span className="text-zinc-300">{text(dvrs.find((dvr) => dvr.id === camera.nvr_id)?.name || camera.nvr_id, "Standalone")}</span><span className="text-white">{camera.channel ? `CH ${camera.channel}` : text(camera.name, "IP Camera")}</span><span className="text-zinc-300">{text(camera.location || camera.name, "Unmapped")}</span><span><span className={`rounded-full border px-2 py-1 text-[10px] uppercase ${tone(s)}`}>{s}</span></span><span className="text-zinc-300">{privacyLabel(camera.privacy_scope || camera.metadata?.privacy_scope)}</span><span className="text-zinc-300">{text(camera.edge_node_id, "No Edge")}</span><span className="text-zinc-500">{when(camera.last_seen_at)}</span><div className="col-span-7 flex flex-wrap gap-2 pt-1"><Button variant="ghost" onClick={() => void openPlayback(camera)} className="gap-2"><PlayCircle className="h-4 w-4" /> Playback</Button><Button variant="ghost" onClick={() => void validate(camera)} className="gap-2"><TestTube2 className="h-4 w-4" /> Validate Stream</Button><Button variant="ghost" onClick={() => void openProfile(camera)} className="gap-2"><Brain className="h-4 w-4" /> AI profile</Button>{validation[camera.id] ? <span className="rounded-full border border-sky-500/20 bg-sky-500/10 px-3 py-2 text-xs text-sky-100">{validation[camera.id]}</span> : null}</div></article>; })}{!filtered.length && !loading ? <p className="p-5 text-sm text-zinc-500">No cameras match this inventory view.</p> : null}</div>
      </section>

      <section className="grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5"><h2 className="text-sm font-semibold text-white">Registered DVR/NVR Sources</h2><div className="mt-4 space-y-2">{dvrs.map((dvr) => <div key={dvr.id} className="rounded-xl border border-white/10 bg-black/15 px-3 py-3"><div className="flex items-center justify-between"><p className="text-sm text-white">{dvr.name}</p><span className={`rounded-full border px-2 py-1 text-[10px] uppercase ${tone(String(dvr.status || "pending").toLowerCase())}`}>{dvr.status || "pending"}</span></div><p className="mt-1 text-xs text-zinc-500">{dvr.brand} · {dvr.ip_address}:{dvr.port} · {dvr.channel_count} channels · {dvr.edge_node_id || "No Edge node"}</p></div>)}{!dvrs.length ? <p className="rounded-xl border border-dashed border-white/10 p-4 text-sm text-zinc-500">No DVR/NVR source imported yet.</p> : null}</div></div><div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5"><h2 className="text-sm font-semibold text-white">Recent camera events</h2><div className="mt-4 space-y-2">{events.slice(0, 8).map((event) => <div key={event.id} className="rounded-xl border border-white/10 bg-black/15 px-3 py-3"><p className="text-sm text-white">{text(event.event_type).replace(/_/g, " ")}</p><p className="mt-1 text-xs text-zinc-500">{event.camera_name || event.camera_id} · {when(event.created_at)} · confidence {event.confidence ?? "n/a"}</p></div>)}{!events.length && !loading ? <p className="rounded-xl border border-dashed border-white/10 p-3 text-sm text-zinc-500">No recent camera events from backend.</p> : null}</div></div></section>
