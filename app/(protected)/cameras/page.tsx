...
export default function CamerasPage() {
  ...
  const [cameraType, setCameraType] = useState<"ip_camera" | "dvr_channel" | "nvr_channel">("ip_camera");
  ...
  async function bind() {
    ...
    try {
      await cameraService.bind({
        estateId: estateId || undefined,
        name: bindName || binding.name || `Camera ${ip}`,
        ip,
        rtsp_url: rtsp,
  ...
  return (
    <div className="space-y-6">
      ...
      {binding ? <div className="fixed inset-0 z-[60] grid place-items-center bg-black/75 p-4 backdrop-blur-sm"><section className="w-full max-w-xl rounded-2xl border border-white/10 bg-zinc-950 p-5"><h2 className="text-lg font-semibold text-white">Bind camera</h2><div className="mt-5 grid gap-3 sm:grid-cols-2"><input value={bindName} onChange={(e) => setBindName(e.target.value)} placeholder="Camera name" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" /><input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Location e.g. Main Gate" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" /><select value={cameraType} onChange={(e) => setCameraType(e.target.value as any)} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none"><option value="ip_camera">IP Camera</option><option value="dvr_channel">DVR Channel</option><option value="nvr_channel">NVR Channel</option></select><input value={credentialRef} onChange={(e) => setCredentialRef(e.target.value)} placeholder="Credential ref" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" /><input value={edgeNodeId} onChange={(e) => setEdgeNodeId(e.target.value)} placeholder="Edge node ID" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" /><input value={dvrId} onChange={(e) => setDvrId(e.target.value)} placeholder="DVR/NVR ID" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" /><input value={channelNumber} onChange={(e) => setChannelNumber(e.target.value)} placeholder="Channel number" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" /></div><p className="mt-3 text-xs text-zinc-500">{ipFromDiscovery(binding)} · {rtspFromDiscovery(binding) || "RTSP unavailable"} · Credentials remain local through credential references.</p><footer className="mt-5 flex justify-end gap-2"><Button variant="ghost" onClick={() => setBinding(null)}>Cancel</Button><Button onClick={() => void bind()} disabled={!ipFromDiscovery(binding) || !rtspFromDiscovery(binding)}><Link2 className="mr-2 h-4 w-4" /> Bind</Button></footer></section></div> : null}
