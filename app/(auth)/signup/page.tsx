...
function StatusDot({ ok }: { ok: boolean | null }) {
  const cls = ok === null ? "bg-zinc-500" : ok ? "bg-emerald-500" : "bg-amber-500";
  const title = ok === null ? "Checking…" : ok ? "Connected" : "Degraded";
  return <span className={cn("h-2 w-2 rounded-full", cls)} title={title} aria-label={title} />;
}
...
function ResendIcon({ className = "" }: { className?: string }) {
  return (
    <svg className={className} width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M20 12a8 8 0 0 1-14.3 5M4 12A8 8 0 0 1 18.3 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
  ...
  return (
    <div className="min-h-screen bg-zinc-950 relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen grid grid-cols-1 lg:grid-cols-12">
        {/* LEFT (simple, statement-making, no cards) */}
        <div className="lg:col-span-7 px-6 py-10 lg:px-14 lg:py-14 flex items-center">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-sm md:text-base text-white/60 leading-relaxed">
              Create a facility operator account to run buildings as infrastructure.
              This console is where operations happen: access control, visitors, work orders,
              service delivery, and resident communications — all under one control plane.
            </p>

            <div className="mt-7 space-y-2 text-sm text-white/55">
              <div className="flex items-start gap-2">
                <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-white/40" />
                <span>Built for real-world facility operations, not “pretty dashboards”.</span>
              </div>
              <div className="flex items-start gap-2">
                <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-white/40" />
                <span>Operator accounts are verified before activation (email OTP).</span>
              </div>
              <div className="flex items-start gap-2">
                <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-white/40" />
                <span>Once created, you’re dropped directly into the command environment.</span>
              </div>
            </div>
        ...
        <div className="lg:col-span-5 px-6 py-10 lg:px-12 lg:py-14 flex items-center justify-center">
          <div className="w-full max-w-md relative">
            ...
            <div className="pointer-events-none absolute -top-6 -right-4 opacity-[0.07]">
            ...
            <div className="flex items-center gap-3">
              <Logo size={34} />
              <div>
                <div className="text-white text-lg font-semibold tracking-tight">Oyi Facility</div>
                <div className="text-[11px] text-white/45">operator onboarding</div>
              </div>
            </div>
            ...
            {/* ✅ IMPORTANT FIX: overflow-hidden clips the OTP panel fully */}
            <div className="mt-6 overflow-hidden">
              ...
              >
                {/* FORM */}
                <div className={cn("w-1/2 pr-4", step === "otp" && "pointer-events-none")}>
                  ...
                  <div className="mt-4 text-[11px] text-white/40">
                    Use your facility operator email. Consumer accounts won’t work here.
                  </div>
                </div>

                {/* OTP (only visible after continue; now clipped until slide) */}
                <div className={cn("w-1/2 pl-4", step === "form" && "pointer-events-none")}>
